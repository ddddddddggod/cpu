...
always @(posedge clk or negedge rstb) begin
    if (!rstb) begin
        intr_captured <= 32'h0;
    end else if (m_state == m_intr) begin
...
always @(*) begin
    m_state_n = m_state;
    case (m_state)
        m_mask: m_state_n = m_idle;
        m_idle: if (interrupt) m_state_n = m_intr;
        m_intr: m_state_n = m_status;
        m_status: begin                
                if (intr_init) begin 
        ...
        m_intr_clr: m_state_n = m_idle;
    endcase
end

//_____________Output logic_________________________________________
assign rw_valid = (m_state == m_status) && (read || write);
...
always @(posedge clk or negedge rstb) begin
    if (!rstb) begin
        ...
        init_r <= (m_state == m_status) && intr_init;
    end
end
assign init = init_r;
